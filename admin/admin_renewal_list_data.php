<?php 
    ...
    if(isset($_REQUEST['submit']))
    {
        ...
        if(move_uploaded_file($_FILES['picture']['tmp_name'], $_SERVER['DOCUMENT_ROOT'].'../admin/assets/image/'.$picture))
        {
            $target_file = $_SERVER['DOCUMENT_ROOT'].'../admin/assets/image/'.$picture;
        ...
        if($result)
            {
                ...
                header("location: admin_renewal_list.php");
                exit(0);
            }
            else
            {
                
                header("location: admin_renewal.php");
                exit(0);
            }
